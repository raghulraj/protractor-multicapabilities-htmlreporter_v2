...
function generateReport(jsonstr, automationHeader) {
    ...
    for (var j = 0; j < jsonstr.length; j++) {
        var browsername = jsonstr[j].description.split('|')[2] + '-' + jsonstr[j].description.split('|')[3];
        browserArray.push(browsername);
    }
    var unique = browserArray.filter(onlyUnique);

    for (var j2 = 0; j2 < jsonstr.length; j2++) {
        themeArray.push(jsonstr[j2].description.split('|')[0]);
    }
    var tunique = themeArray.filter(onlyUnique);

    for (var j3 = 0; j3 < jsonstr.length; j3++) {
        testrunArray.push(jsonstr[j3].description.split('|')[1]);
    }
    ...
    for (var q = 0; q < jsonstr.length; q++) {
        var browsername = jsonstr[q].description.split('|')[2] + '-' + jsonstr[q].description.split('|')[3];
        runningArray.push(browsername + "~" + jsonstr[q].description.split('|')[0] + "~" + jsonstr[q].description.split('|')[1]);
    ...
    diffArray = arr_diff(runningArray, comboArray);
    if (diffArray.length > 0) {
        for (var pk = 0; pk < diffArray.length; pk++) {
            var missingArray = new Array();
            missingArray = diffArray[pk].split("~");
            var missingbrowser = missingArray[0];
            var missingtheme = missingArray[1];
            var missingtest = missingArray[2];
            testArray.push({
                theme: missingtheme,
                testName: missingtest,
    ...
    for (var i = 0; i < resultq.length; i++) {
        ...
        result += '<tr><td class="brk1"><b>' + tunique[i].toUpperCase(); + '</b></td></tr>';
        ...
        for (var i2 = 0; i2 < resultq[i].length; i2++) {

            if (!dummy[resultq[i][i2].testName]) {
                dummy[resultq[i][i2].testName] = {};
            }
            if (!dummy[resultq[i][i2].testName].browser) {
                dummy[resultq[i][i2].testName].browser = {};
            }

            dummy[resultq[i][i2].testName].browser[resultq[i][i2].browser] = resultq[i][i2].res;
        ...
        var pcounter = 1;
        var fcounter = 1;
        for (var inc1 in resultq[i]) {
            for (var inc2 in lk) {
                bbk = lk[inc2];
                if (bbk == resultq[i][inc1].browser) {
                    browserCount.push(bbk);
                    if (resultq[i][inc1].res == "true") {
                        trueCount.push(bbk);
                    }
                    if (resultq[i][inc1].res == "false") {
                        falseCount.push(bbk);
                    }
                    if (resultq[i][inc1].res == "Skipped") {
